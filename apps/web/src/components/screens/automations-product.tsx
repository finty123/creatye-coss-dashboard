...
import { ConfirmActions, ProductEmpty, ProductModal, ProductNotice, ProductPage, ProductPagination, ProductTabs } from "@/components/product/product-ui";

function matchesAutomation(item: AutomationFixture, tab: string, paused: Set<string>) {
...
export function AutomationsProduct() {
  ...
  const visible = useMemo(() => automationFixtures.filter((item) => matchesAutomation(item, tab, paused)), [paused, tab]);
  ...
  return <ProductPage eyebrow="Automação" title="Mantenha cada conversa em movimento" description="Crie, acompanhe e otimize fluxos em todos os canais conectados." actions={<Button variant="primary" onClick={() => setCreateOpen(true)}><Plus size={14} /> Nova automação</Button>}>
    ...
    <div className="product-toolbar"><select className="product-compact-select" aria-label="Filtrar página"><option>Todas as páginas</option><option>Creatye Brasil</option><option>Academy Launch</option></select><span className="product-toolbar-spacer" /><select className="product-compact-select" aria-label="Ordenar"><option>Atualizadas recentemente</option><option>Mais execuções</option><option>Maior conversão</option></select></div>
