...
export function PagesCalibration() {
  ...
  const visible = useMemo(() => connectedPages.filter((item) => (status === "all" || item.status === status) && (platform === "all" || item.platform === platform)).sort((a, b) => sort === "name" ? a.name.localeCompare(b.name) : a.lastSync.localeCompare(b.lastSync)), [platform, sort, status]);
